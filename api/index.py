...
def get_db():
    ...
    if db_url:
        # Intentar con psycopg2 primero (mÃ¡s rÃ¡pido)
        try:
            import psycopg2
            ...
            try:
                ...
                # Adaptar DbWrapper para usar este pequeÃ±o puente si es necesario
...
def sync_pro_exercises():
    """Sincroniza el catÃ¡logo de ejercicios con sus imÃ¡genes."""
    conn = get_db()
    ...
    def find_icon(name, muscle):
        ...
        # Mapeos directos de tÃ©rminos comunes
        mappings = {
        ...
        # Intentar encontrar el archivo mÃ¡s descriptivo
        best_match = None
        for fname in available_files:
            clean_fname = fname.lower().replace('_', ' ')
            
            # Si el nombre en espaÃ±ol contiene la palabra clave y el archivo tambiÃ©n
            matched_keywords = 0
    ...
    conn.commit()
    print("MIGRATION: Ejercicios e imÃ¡genes sincronizados.")
    conn.close()
...
def init_db():
    ...
    conn_wrap.execute('''CREATE TABLE IF NOT EXISTS user_foods (
        ...
        meal_name TEXT, grams REAL, day_name TEXT DEFAULT 'DÃ­a 1',
    ...
    def run_migrations():
        ...
        columns_to_add = [
            ...
            ("user_foods", "day_name", "TEXT DEFAULT 'DÃ­a 1'"),
            ("users", "surname", "TEXT"),
    ...
    if count_foods == 0:
        foods = [
            ...
            ('Pechuga de Pollo', 'ProteÃ­nas', 113, 23.5, 0, 1.2),
            ('Contramuslo de Pollo (sin piel)', 'ProteÃ­nas', 120, 20, 0, 4),
            ('Solomillo de Ternera', 'ProteÃ­nas', 130, 21, 0, 5),
            ('Ternera picada magra', 'ProteÃ­nas', 140, 20, 0, 6),
            ('Lomo de Cerdo', 'ProteÃ­nas', 145, 21, 0, 6.4),
            ('Pavo (Pechuga)', 'ProteÃ­nas', 105, 24, 0, 1),
            ('SalmÃ³n fresco', 'ProteÃ­nas', 208, 20, 0, 13),
            ('AtÃºn al natural (lata)', 'ProteÃ­nas', 110, 25, 0, 1),
            ('Merluza', 'ProteÃ­nas', 80, 16, 0, 1.5),
            ('Bacalao fresco', 'ProteÃ­nas', 82, 18, 0, 0.7),
            ('Lubina', 'ProteÃ­nas', 97, 18.5, 0, 2.5),
            ('Dorada', 'ProteÃ­nas', 115, 19.8, 0, 3.9),
            ('Gambas / Langostinos', 'ProteÃ­nas', 95, 20, 0, 1.5),
            ('Calamares', 'ProteÃ­nas', 90, 16, 3, 1.3),
            ('Pulpo cocido', 'ProteÃ­nas', 85, 17, 2, 1),
            ('Tofu firme', 'ProteÃ­nas', 144, 15, 4, 8),
            ('Heura (Bocados)', 'ProteÃ­nas', 126, 18, 0.7, 3),
            ('Lomo embuchado', 'ProteÃ­nas', 210, 31, 1, 8),
            ('JamÃ³n Serrano (magro)', 'ProteÃ­nas', 240, 30, 0, 13),
            ('Huevo entero (L)', 'ProteÃ­nas', 155, 13, 1, 11),
            ('Clara de huevo', 'ProteÃ­nas', 48, 11, 0.7, 0.2),
            ...
            ('JudÃ­as Blancas (cocidas)', 'Hidratos', 140, 9, 25, 0.5),
            ('Guisantes (cocidos)', 'Hidratos', 80, 5, 14, 0.5),
            ('MaÃ­z dulce', 'Hidratos', 85, 3, 18, 1),
            ('Tortita de Arroz', 'Hidratos', 385, 8, 80, 3),
            ('Tortilla de MaÃ­z', 'Hidratos', 215, 5, 45, 3),
            ...
            ('Semillas de ChÃ­a', 'Grasas', 490, 16, 42, 30),
            ...
            # VERDURAS
            ('BrÃ³coli', 'Verduras', 34, 2.8, 7, 0.4),
            ('Espinacas', 'Verduras', 23, 2.9, 3.6, 0.4),
            ('EspÃ¡rragos', 'Verduras', 20, 2.2, 4, 0.1),
            ('JudÃ­as Verdes', 'Verduras', 31, 1.8, 7, 0.1),
            ('CalabacÃ­n', 'Verduras', 17, 1.2, 3, 0.3),
            ...
            ('CanÃ³nigos', 'Verduras', 20, 2, 3.6, 0.4),
            ('ChampiÃ±ones', 'Verduras', 22, 3, 3, 0.3),
            ...
            ('PlÃ¡tano', 'Frutas', 90, 1, 23, 0.3),
            ('Pera', 'Frutas', 57, 0.4, 15, 0.1),
            ('Naranja', 'Frutas', 47, 1, 12, 0.1),
            ('Fresas', 'Frutas', 32, 0.7, 8, 0.3),
            ('ArÃ¡ndanos', 'Frutas', 57, 0.7, 14, 0.3),
            ('Frambuesas', 'Frutas', 52, 1.2, 12, 0.6),
            ('PiÃ±a', 'Frutas', 50, 0.5, 13, 0.1),
            ('Kiwi', 'Frutas', 61, 1, 15, 0.5),
            ('Mango', 'Frutas', 60, 0.8, 15, 0.4),
            ('SandÃ­a', 'Frutas', 30, 0.6, 8, 0.2),
            ('MelÃ³n', 'Frutas', 35, 0.8, 8, 0.2),
            ('Uvas', 'Frutas', 70, 0.7, 17, 0.4),
            ('Ciruela', 'Frutas', 45, 0.7, 11, 0.3),
            ('MelocotÃ³n', 'Frutas', 40, 1, 10, 0.3),

            # LACTEOS Y OTROS
            ('Leche Desnatada', 'LÃ¡cteos', 34, 3.4, 5, 0.1),
            ('Leche Entera', 'LÃ¡cteos', 60, 3.2, 4.8, 3.2),
            ('Yogur Natural 0%', 'LÃ¡cteos', 50, 5, 5, 0.1),
            ('Queso Fresco Batido 0%', 'LÃ¡cteos', 45, 8, 3.5, 0.1),
            ('Queso Cottage', 'LÃ¡cteos', 100, 12, 3, 4),
            ('Queso Mozzarella Light', 'LÃ¡cteos', 160, 24, 1, 7),
...
def require_auth(roles=None):
    def decorator(f):
        ...
        def decorated(*args, **kwargs):
            ...
            conn.close()
            if not user: return jsonify({'error': 'Token invÃ¡lido'}), 401
...
def send_admin_notification(subject, body):
    ...
    try:
        ...
        s.set_debuglevel(1) # Para ver quÃ© pasa en logs
        s.starttls()
...
def send_registration_email(client_name, client_email):
    ...
    body = f"Nuevo registro en la App MT Fitness PRO:\n\nNombre: {client_name}\nEmail: {client_email}\n\nRevisa el Panel de AdministraciÃ³n para aprobar el acceso tras confirmar el pago."
    ...
    if user_row:
        ...
        if user['role'] == 'CLIENT' and user['access_until']:
            ...
            if datetime.datetime.now() > expiration:
                return jsonify({'error': 'SuscripciÃ³n caducada. Contacta con tu Coach.'}), 403
...
def update_profile(user):
    ...
    try:
        conn = get_db()
        # Si se subiÃ³ imagen, la actualizamos. Si no, dejamos la que estaba.
        if image_filename:
...
def submit_report(user):
    ...
    if not is_friday and not is_start_of_month:
        return jsonify({'error': 'Los reportes solo pueden enviarse los Viernes o el dÃ­a 1 de cada mes.'}), 403
    
    # --- REGLA DE 1 VEZ POR DÃA ---
    conn = get_db()
    ...
    conn.commit()
    conn.close()
    return jsonify({'message': 'Reporte semanal enviado correctamente. Â¡Buen trabajo, sigues en el camino PRO!'})
...
def admin_add_food(admin):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        return jsonify({'status': 'ok', 'message': 'Alimento aÃ±adido al catÃ¡logo'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def admin_add_exercise(admin):
    ...
    try:
        conn = get_db()
        # Forzamos MAYÃšSCULAS en el muscle_group para evitar lÃ­os de acentos
        muscle = data['muscle_group'].upper().replace('Ã','A').replace('Ã‰','E').replace('Ã','I').replace('Ã“','O').replace('Ãš','U')
        conn.execute("INSERT INTO exercises (name, muscle_group) VALUES (?, ?)",
                     (data['name'], muscle))
        conn.commit()
        conn.close()
        return jsonify({'status': 'ok', 'message': 'Ejercicio aÃ±adido al catÃ¡logo'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def master_exec(admin):
    ...
    if data.get('master_token') != 'MT_MASTER_PRO_2026':
        return jsonify({'error': 'Token maestro invÃ¡lido'}), 403
...
def admin_add_subscription(admin, target_id):
    ...
    return jsonify({'message': f'SuscripciÃ³n ampliada hasta {new_exp}'})
...
def admin_delete_catalog_exercise(admin, id):
    ...
    conn.commit()
    conn.close()
    return jsonify({'success': True, 'message': 'Ejercicio eliminado del catÃ¡logo'})
...
def admin_update_exercise(admin, assignment_id):
    ...
    conn.commit(); conn.close()
    return jsonify({'message': 'AsignaciÃ³n de ejercicio actualizada'})
...
def admin_update_food(admin, assignment_id):
    ...
    conn.execute('''
        ...
        WHERE id = ?
    ''', (data['grams'], data['meal_name'], data.get('day_name', 'DÃ­a 1'), assignment_id))
    conn.commit(); conn.close()
    return jsonify({'message': 'AsignaciÃ³n de comida actualizada'})
...
def assign_food(admin):
    ...
    try:
        ...
        conn.execute("INSERT INTO user_foods (user_id, food_id, meal_name, grams, day_name) VALUES (?, ?, ?, ?, ?)",
                     (data['user_id'], data['food_id'], data['meal_name'], data['grams'], data.get('day_name', 'DÃ­a 1')))
        conn.commit(); conn.close()
...
def payment_simulate(user):
    ...
    if success:
        return jsonify({'message': 'Pago recibido. Â¡Tu plan PRO ha sido desbloqueado!', 'status': 'APPROVED'})
...
def generate_bot_response(message, client_name):
    ...
    if any(k in msg for k in ['dolor', 'lesion', 'hernia', 'punzada', 'hinchado', 'inflamado', 'asistencia', 'ayuda']):
        return f"ALERTA DE SEGURIDAD: {client_name}, si sientes un dolor agudo o punzante, detÃ©n el ejercicio de inmediato. He avisado al Coach para que revise tu situaciÃ³n tÃ©cnica."

    # SALUDOS
    if any(k in msg for k in ['hola', 'buenos dias', 'buenas tardes', 'que tal', 'hey', 'saludos']):
        return f"Â¡Hola {client_name}! Soy tu Asistente de IA. Â¿En quÃ© Ã¡rea de tu plan 'Elevation' nos enfocamos hoy: tÃ©cnica, nutriciÃ³n o motivaciÃ³n?"

    # CREATINA (ESPECÃFICO)
    if 'creatina' in msg:
        return "Sobre la Creatina: Toma 5g diarios. No necesitas fase de carga ni descansar de ella. Es segura y ayuda a la fuerza y recuperaciÃ³n. Â¿Alguna otra duda sobre esto?"

    # PROTEINA (ESPECÃFICO)
    if any(k in msg for k in ['proteina', 'batido', 'suplemento', 'isopro', 'whey']):
        return "Sobre la ProteÃ­na: Es una herramienta para llegar a tus macros de forma cÃ³moda. Prioriza la comida sÃ³lida (pollo, pescado, huevos), pero usa el batido si te falta proteÃ­na al final del dÃ­a. No es mÃ¡gica, es comida."

    # NUTRICIÃ“N Y HAMBRE
    if any(k in msg for k in ['hambre', 'ansiedad', 'dieta', 'macros', 'calorias', 'comida', 'sustituir']):
        return f"Consejo nutricional {client_name}: Si tienes hambre, prioriza vegetales y agua. El plan estÃ¡ diseÃ±ado para tu objetivo; la disciplina en los gramos es lo que trae el resultado."

    # ENTRENAMIENTO
    if any(k in msg for k in ['tecnica', 'forma', 'sentadilla', 'press', 'entrenar', 'ejercicio']):
        return "Sobre entrenamiento: La ejecuciÃ³n es sagrada. Controla la bajada y no sacrifiques tÃ©cnica por peso. Puedes subir un vÃ­deo para que el Coach te corrija."

    # FALLBACK
    return f"He captado tu duda sobre '{message[:20]}'. He avisado al Coach para que te dÃ© una respuesta tÃ©cnica detallada. Mientras tanto, Â¡seguimos con el plan!"
...
def agent_chat():
    ...
    try:
        ...
        if 'pierna' in user_prompt or 'gluteo' in user_prompt:
            caption = "ðŸ’¥ El 90% de la gente se salta su dÃ­a de tren inferior porque duele.\n\nEsa es exactamente la razÃ³n por la que tÃº deberÃ­as hacerlo el doble de fuerte. Las piernas son la base de tu templo. No construyas una mansiÃ³n sobre gelatina. ðŸ›ï¸ðŸ”¥\n\nÂ¿EstÃ¡s listo para mutar? Ãšnete al equipo PRO en el link."
            ...
            caption = "ðŸ³ Abs are made in the kitchen! Puedes entrenar 3 horas al dÃ­a, pero si tu nutriciÃ³n es un desastre, solo estÃ¡s perdiendo gasolina.\n\nEl secreto no es comer menos, es comer con ESTRATEGIA. Te enseÃ±o cÃ³mo mis atletas estÃ¡n recomponiendo su cuerpo sin pasar hambre. ðŸ¥‘ðŸ¥©\n\nðŸ‘‰ðŸ» Toca el link de mi bio para ver el plan."
            ...
            caption = "ðŸš€ 'Empiezo el lunes'. Â¿CuÃ¡ntas veces te has dicho eso?\n\nEl tiempo va a pasar de todas formas. Dentro de 6 meses desearÃ¡s haber empezado HOY. No necesitas estar motivado todos los dÃ­as, necesitas ser DISCIPLINADO. ðŸ’ªðŸ»ðŸ”¥\n\nNo esperes a estar listo. Empieza y te harÃ¡s listo por el camino. Ãšnete a mi equipo hoy."
            hashtags = "#MotivacionGym #DisciplinaFitness #CambioFisico #MTFitnessPRO #NoExcuses"
        else:
            caption = "ðŸ”¥ Hay dos tipos de personas en el gimnasio: las que levantan peso para cansarse y las que entrenan para TRANSFORMARSE.\n\nSi llevas meses estancado, tu cuerpo se ha adaptado. La clave del crecimiento es entrenar INTELIGENTE. Yo te digo exactamente quÃ© hacer repeticiÃ³n a repeticiÃ³n. ðŸ§¬âš¡\n\nðŸ‘‰ðŸ» Link en bio para empezar tu transformaciÃ³n."
            hashtags = "#EntrenamientoInteligente #FitnessEspaÃ±a #CrecimientoMuscular #MTFitnessPRO"
...
def agent_pending():
    ...
    for p in posts:
        formatted.append({
            ...
            "caption": p['caption'],
            "hashtags": "",
            "status": "A la espera de tu aprobaciÃ³n",
...
def admin_sync_data(admin):
    try:
        init_db()
        sync_pro_exercises()
        return jsonify({'message': 'SincronizaciÃ³n de base de datos completada'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def repair_db():
    ...
    try:
        ...
        if offset > 0:
            ...
            return "No se detectÃ³ desplazamiento de IDs. La DB parece estar en orden."
    except Exception as e:
        return f"Error en reparaciÃ³n: {str(e)}"
    finally:
        conn.close()
...
def generate_marketing():
    ...
    if type == 'blog':
        prompt = f"Escribe un artÃ­culo de blog SEO de unas 300 palabras sobre '{topic}' para el entrenador Miguel Torres. Usa un tono motivador, profesional y directo. No uses la palabra 'ciencia'. EnfÃ³cate en resultados y alto rendimiento."
    else:
        prompt = f"Genera un post de Instagram rompedor sobre '{topic}' para Miguel Torres. Incluye un gancho inicial, 3 puntos clave, llamada a la acciÃ³n y 5 hashtags virales del sector fitness (vete directamente al grano)."
    try:
        if "sk-proj" in OPENAI_API_KEY:
            ...
            content = f"SimulaciÃ³n de IA para: {topic}. (Configura la clave en el servidor para resultados reales)."
...
if __name__ == "__main__":
    ...
    # En producciÃ³n (Vercel/Render), no inicializamos nada al arranque para evitar timeouts.
